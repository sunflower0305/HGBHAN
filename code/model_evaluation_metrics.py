...
def cv_model_evaluate(interaction_matrix, predict_matrix, train_matrix):
    ...
    unique, counts = np.unique(real_score, return_counts=True)
    label_counts = dict(zip(unique, counts))
    if len(label_counts) == 2 and min(label_counts.values()) / max(label_counts.values()) < 0.1:
        X = np.array([[i] for i in predict_score])
        y = real_score
        smote = SMOTE()
        X_resampled, y_resampled = smote.fit_resample(X, y)
        predict_score = X_resampled.flatten()
        real_score = y_resampled
